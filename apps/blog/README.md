...
About 페이지는 Notion 포스트 중 `Status`가 **About**인 게시물을 자동으로 가져와 보여주는 기능입니다.

### 설정 방법

1. Notion 블로그 데이터베이스에서 새 페이지를 생성합니다.
2. `Status` 속성을 **About**으로 설정합니다. (태그 설정은 불필요합니다)

### 참고사항

- `Status`가 **About**인 포스트가 여러 개일 경우, 가장 최근에 작성된 포스트가 표시됩니다.
- `Status`가 **About**인 포스트가 하나도 없을 경우, 헤더의 About 링크는 자동으로 숨겨집니다.
