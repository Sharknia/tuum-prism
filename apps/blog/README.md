...
Notion 데이터베이스의 `상태` 속성으로 포스트 발행 상태를 관리합니다:

| 상태          | 설명                          |
| ------------- | ----------------------------- |
| `Writing`     | 작성 중 (블로그에 표시 안 됨) |
| `Ready`       | 작성 완료, 발행 대기          |
| `Updated`     | **발행됨** (블로그에 표시)    |
| `About`       | About 페이지에 표시           |
| `ToBeDeleted` | 삭제 예정                     |
...
- `상태`가 **About**인 포스트가 About 페이지에 표시됩니다.
- 여러 개일 경우, 가장 최근에 작성된 포스트가 표시됩니다.
- 하나도 없을 경우, 헤더의 About 링크는 자동으로 숨겨집니다.
